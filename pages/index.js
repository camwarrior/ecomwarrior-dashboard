...
import { getResumenMensual, getMovimientos, getConfig } from "../lib/sheets";
...
export default function Dashboard({ resumen, movimientos, config, lastUpdated }) {
  // ── Totales anuales ──────────────────────────────────────
  const totalIngresos   = resumen.reduce((s, r) => s + r.ingresos, 0);
  const totalGastos     = resumen.reduce((s, r) => s + r.totalGastos, 0);
  const utilidadNeta    = totalIngresos - totalGastos;
  const totalMetaAds    = resumen.reduce((s, r) => s + r.metaAds, 0);
  const totalSoftwareIA = resumen.reduce((s, r) => s + r.softwareIA, 0);

  // ── Saldo por cuenta ────────────────────────────────────
  const saldoMercury = movimientos
    .filter(m => m.cuenta === "Mercury")
    .reduce((s, m) => s + (m.tipo === "Ingreso" ? m.montoUSD : -Math.abs(m.montoUSD)), 0);

  const saldoSlash = movimientos
    .filter(m => m.cuenta === "Slash")
    .reduce((s, m) => s + (m.tipo === "Ingreso" ? m.montoUSD : -Math.abs(m.montoUSD)), 0);
      ...
      <div style={{ background: "#0a0a0a", minHeight: "100vh", color: "#e8e8e8" }}>
        ...
        <main style={{ padding: "32px 40px", maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 40 }}>
            <StatCard label="Ingresos totales" value={totalIngresos} color="green" />
            <StatCard label="Gastos totales"   value={totalGastos}   color="red" />
                      ...
                      <tr key={i} style={{ borderBottom: "1px solid #161616" }}>
                        ...
                        <td style={tdStyle}>
                          <span style={{
                            ...
                            background: m.tipo === "Ingreso" ? "#16391f" : "#3b1a1a",
                        ...
                        <td style={{ ...tdStyle, color: m.tipo === "Ingreso" ? "#4ade80" : "#f87171", fontFamily: "'DM Mono'" }}>
                          {m.tipo === "Ingreso" ? "+" : "-"}{fmt(Math.abs(m.montoUSD))}
                        </td>
...
export async function getServerSideProps() {
  try {
    const [resumen, movimientos, config] = await Promise.all([
      getResumenMensual(),
      getMovimientos(),
      getConfig(),
    ]);
